...
// Ensure schemaVersion echo field matches underlying schema version if present
if(catalog.schemaVersion && catalog.schemaVersion !== schema['x-schema-version']){
  console.error(`catalog.schemaVersion (${catalog.schemaVersion}) != schema.x-schema-version (${schema['x-schema-version']})`);
  process.exit(4);
} else if(!catalog.schemaVersion){
  catalog.schemaVersion = schema['x-schema-version'];
...
console.log(`Instruction catalog valid. Entries: ${catalog.entries.length}. Catalog v${catalog.version}; Schema ${schema['x-schema-version'] || schema['$id']}`);
