...
// Helper to start enterprise server (includes metrics HTTP) so we can query /api/metrics
function startServer(envExtra={}) {
  ...
  // Prefer server.js if present else index.js
...
function callTool(proc, id, command){
  send(proc, { jsonrpc:'2.0', id, method:'tools/call', params:{ name:'run-powershell', arguments:{ command, confirmed:true } } });
}
...
async function waitForSamples(min=1, timeoutMs=20000){
  ...
  while(Date.now()-start < timeoutMs){
    try {
      ...
      if(snap.psSamples && snap.psSamples >= min){ return snap; }
...
describe('ps metrics aggregation', ()=>{
  test('exposes aggregated ps metrics after a few invocations', async()=>{
    ...
    server.stdout.on('data', buf=>{
      ...
      for(const l of lines){
        try { const obj = JSON.parse(l); if(obj.result){ responses++; toolResults.push(obj.result); } } catch {/* ignore non-json */}
      }
    });
    try {
      // Initialize handshake (some server variants require it)
      ...
      // Fire several quick invocations
      ...
      expect(snap.psCpuSecAvg).toBeLessThan(30);
      expect(snap.psWSMBAvg).toBeGreaterThan(0);
      expect(snap.psWSMBAvg).toBeLessThan(2048);
    } finally {
      server.kill();
    }
  }, 50000);
