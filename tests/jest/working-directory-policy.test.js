...
describe('working_directory_policy tool', () => {
  test('enforces disallowed working directory', async () => {
    ...
    rpc(srv,'tools/call',{ name:'working_directory_policy', arguments:{ action:'set', enabled:true, allowedWriteRoots:[allowed] }},'set');
    // Attempt run outside allowed root (process.cwd())
    rpc(srv,'tools/call',{ name:'run_powershell', arguments:{ command:'Write-Output "wdtest"', working_directory: process.cwd(), confirmed:true }},'run');
    // Wait a bit
