...
export class EnterprisePowerShellMCPServer {
  ...
  private async handleToolCall(name:string, args:any, requestId:string){ const started=Date.now(); let published=false; const publish=(success:boolean, note?:string)=>{ if(published) return; const duration=Date.now()-started; if(name==='run-powershell'||name==='run-powershellscript'){ published=true; return; } try { metricsHttpServer.publishExecution({ id:`tool-${Date.now()}-${Math.random().toString(36).slice(2,8)}`, level: success?'SAFE':'UNKNOWN', durationMs: duration, blocked:false, truncated:false, timestamp:new Date().toISOString(), preview:`${name}${note? ' '+note:''}`.substring(0,120), success, exitCode:null, toolName:name }); } catch{} try { metricsRegistry.record({ level: success?'SAFE':'UNKNOWN', blocked:false, durationMs: duration, truncated:false }); } catch{} published=true; };
    try {
      switch(name){
        case 'emit-log': {
