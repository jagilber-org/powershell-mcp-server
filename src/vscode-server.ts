...
// TypeScript interfaces for better type safety and maintainability
interface SecurityAssessment {
    level: 'SAFE' | 'RISKY' | 'DANGEROUS' | 'CRITICAL' | 'BLOCKED' | 'UNKNOWN';
    risk: 'LOW' | 'MEDIUM' | 'HIGH' | 'EXTREME' | 'FORBIDDEN';
    reason: string;
    color: 'GREEN' | 'YELLOW' | 'MAGENTA' | 'RED' | 'CYAN';
    blocked?: boolean;
...
export class VSCodePowerShellMcpServer {
    ...
    validateAuthKey(providedKey?: string): boolean {
    ...
    async executePowerShellCommand(command: string, timeout: number = 300000, workingDirectory?: string, securityAssessment: SecurityAssessment | null = null): Promise<ExecutionResult> {
