...
export class PowerShellMcpServer {
  ...
  private httpPort = 8384; // Different port than user's server
  private httpKey = 'mcp-powershell-12345';
  private isHttpServerRunning = false;
  ...
  private async executePowerShellCommand(command: string, timeout: number): Promise<any> {
  ...
  private async executePowerShellScript(script: string, timeout: number): Promise<any> {
    return new Promise((resolve, reject) => {
  ...
  private async executePowerShellFile(filePath: string, parameters?: Record<string, any>, timeout: number = 30000): Promise<any> {
  ...
  private async executePowerShellViaHttp(command: string, timeout: number): Promise<any> {
